...
graph1 = create_graph(rectangles_areas_list[0])
graph2 = create_graph(rectangles_areas_list[1])

angles_coordinates = []
angles_coordinates1 = []
for i in range(2, 6):
    console_log(f"Iterations: {i}")
    angles_coordinates1 = push(find_outer_angles_coordinates(graph1, 6, i, img, quality), angles_coordinates1)

angles_coordinates.extend(angles_coordinates1)
...
for i in range(2, 6):
    console_log(f"Iterations: {i}")
    angles_coordinates1 = push(find_outer_angles_coordinates(graph2, 6, i, img, quality), angles_coordinates1)

angles_coordinates.extend(angles_coordinates1)

console_log("angles coordinates")

graph2.angles_coordinates = []
graph2.angles_coordinates = angles_coordinates1[-1]
...
cv2.imwrite("data/modified_polygon11.jpg", modified_img)
...
cv2.imwrite("data/modified1.jpg", modified_img1)
...
for i in range(2, 6):
    ...
    console_log(f"Iterations: {i}")
    middle_coordinates = find_outer_angles_coordinates(graph, 2, i, modified_img, quality)
...
coordinates_to_cut = find_coordinates_to_cut(graph2.angles_coordinates, img)
(min_x, min_y) = coordinates_to_cut[1]
...
cv2.imwrite("data/modified2.jpg", modified_img2)


modified_img = modified_img2
console_log(modified_img.shape)
console_log()
...
graph2.angles_coordinates.extend(angles_coordinates2[-1])
...
console_log(graph1.angles_coordinates)
console_log(graph2.angles_coordinates)


sides_length = count_length_of_sides_of_complete_graph(graph1.angles_coordinates)
graph1.sides_length = sides_length
console_log("length of sides")
console_log(sides_length)
console_log()


sides_length = count_length_of_sides_of_complete_graph(graph2.angles_coordinates)
graph2.sides_length = sides_length
console_log("length of sides")
console_log(sides_length)
console_log()


console_log(check_similarity_of_complete_graphs(graph1, graph2))

...
def group_by_minimum_distance(graphs_list, img):
    ...
    clusters = []
    min_distance1 = width
    min_distance_btwn_clusters = width

    print("graphs_list length")
    ...
    for value in graphs_list:
        graphs_list_copy.append(value)


    for i in range(len(graphs_list) - 1):
        for j in range(len(graphs_list_copy) - 1):
            graphs_list[i]
            graphs_list_copy[j]
            ...
            for value in graphs_list_copy[j].edges.keys():
                (x, y) = value
                if x < min_x_2:
                    min_x_2 = x
                if x > max_x_2:
    ...
    for i in range(len(graphs_list) - 1):
        for j in range(len(graphs_list_copy) - 1):
            ...
            if min(distances) > 0:
                if min(distances) > 2 * min_distance1:
                    if min(distances) < min_distance_btwn_clusters:
                        min_distance_btwn_clusters = min(distances)


    print("distance between clusters")
    print(min_distance_btwn_clusters)
    print()
    ...
    for i in range(len(graphs_list_copy)):
        for value in graphs_list_copy[i].edges.keys():
            (x, y) = value
            ...
            indexes_dict[graphs_list_copy[i]] = min_x


    indexes_dict_values = []

    for value in indexes_dict.values():
        indexes_dict_values.append(value)

    indexes_dict_values.sort()

    indexes_dict2 = {}

    for i in range(len(indexes_dict_values)):
        value2 = indexes_dict_values[i]

        for key, value in indexes_dict.items():
            if value == value2:
                indexes_dict2[key] = value


    indexes_dict = indexes_dict2



    min_x_coordinates = []

    for value in indexes_dict2.values():
        min_x_coordinates.append(value)
    ...
    for value in indexes_dict2.keys():
        graphs_list_copy2.append(value)
    ...
    for i in range(len(graphs_list_copy2) - 1):
        graphs_list_copy2[i]

        min_x_1 = width
        max_x_1 = 0

        for value in graphs_list_copy2[i].edges.keys():
            (x, y) = value
            if x < min_x_1:
                min_x_1 = x
            if x > max_x_1:
        ...
        for value in graphs_list_copy2[i+1].edges.keys():
            (x, y) = value
            if x < min_x_2:
                min_x_2 = x
            if x > max_x_2:
        ...
        if abs(min_distance1 - distance) > abs(min_distance_btwn_clusters - distance):
            number_of_clusters += 1
    ...
    for i in range(len(graphs_list)-1):
        ...
        if abs(min_distance1 - min(distances)) < abs(min_distance_btwn_clusters - min(distances)):
            clusters[cluster_index].append(graphs_list[i])
            clusters[cluster_index].append(graphs_list[i+1])
        elif abs(min_distance1 - min(distances)) > abs(min_distance_btwn_clusters - min(distances)):

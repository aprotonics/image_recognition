...
cut_rectangle_carcass_angle = utils.cut_rectangle_carcass_angle
...
for p in range(3, 21):
    img_name = f"data/rectangle{p}.jpg"
    ...
    cv2.imwrite(f"data/modified_rectangle{p}.jpg", modified_img)
    ...
    coordinates = find_corners_coordinates_of_rectangle(angles_coordinates)
    coordinates = sort_corners_coordinates_of_rectangle(coordinates)

    images = [img for i in range(4)]
    images2 = []


    for i in range(length(images)):
        img2 = cut_rectangle_carcass_angle(coordinates[i], images[i])
        images2.append(img2)
    ...
    for i in range(length(images)):
        ...
        cv2.imwrite(f"data/cache/modified_img{p}_{i+3}.jpg", img)

    isolated_subgraphs_counts1 = []

    for i in range(4):
        img_name = f"data/cache/modified_img{p}_{i+3}.jpg"
        ...
        isolated_subgraphs_counts1 = push(isolated_subgraphs_count, isolated_subgraphs_counts1)

        console_log(isolated_subgraphs_count)
        console_log()

    isolated_subgraphs_counts = push(isolated_subgraphs_counts1, isolated_subgraphs_counts)

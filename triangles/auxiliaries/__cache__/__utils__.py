...
def find_outer_angles_coordinates_of_triangle(graph, angles_number, iterations, img, quality):
    ...
    for key in graph.edges.keys():
        neighbors_count = count_neighbors(graph, key, iterations, img, quality)
        number_of_neighbors[key] = neighbors_count

    number_of_neighbors_copy = number_of_neighbors.copy()

    while len(angles_coordinates_dict) < angles_number:
        min_value = max(number_of_neighbors_copy.values())

        for key, value in number_of_neighbors_copy.items():
            if value == min_value:
                min_value_key = key
        ...
        number_of_neighbors_copy.pop(min_value_key)
    
    if len(angles_coordinates_dict) == angles_number:
        additional_min_value = max(number_of_neighbors_copy.values())
    ...
    while additional_min_value == angles_coordinates_dict_values[values_length-1] and len(number_of_neighbors_copy) >= 1:
        ...
        number_of_neighbors_copy.pop(additional_min_value_key)
        if len(number_of_neighbors_copy.values()) > 0:
            additional_min_value = max(number_of_neighbors_copy.values())
    ...
    search_radius = iterations * 2                   # > iterations
    angles_regions = []
    visited = []
...
def find_inner_angles_coordinates_of_triangle(graph, angles_number, iterations, img, quality):
    ...
    for key in graph.edges.keys():
        in_to_out_ratio = count_neighbors2(graph, key, iterations, img, quality)
    ...
    print(len(angles_coordinates_dict.keys()))
...
def count_neighbors(graph, start, iterations, img, quality):
    graph = graph
    start = start
    iterations = iterations
    img = img
...
def find_inner_angles_coordinates(graph, angles_number, iterations, img, quality):
    ...
    for key, value in in_to_out_ratio_dict.items():
        if value > 0:
            ...
            if not (value_to_reduce - value_to_reduce * quad_diff <= value <= value_to_reduce + value_to_reduce * quad_diff):
                angles_coordinates_dict[key] = value
...
def count_neighbors2_in_triangle(graph, start, iterations, img, quality):
    graph = graph
    start = start
    iterations = iterations
    img = img
...
def count_neighbors2(graph, start, iterations, img, quality):
    graph = graph
    start = start
    iterations = iterations
    img = img
    ...
    if neighbors_in_border_count > 0:
        in_to_out_ratio = neighbors_out_of_border_count / neighbors_in_border_count
...
def check_in_bounds(coordinates, img):
    (x, y) = coordinates
    width = img.shape[0]
    height = img.shape[1]
